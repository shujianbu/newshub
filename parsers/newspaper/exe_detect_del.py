...
db_query_distinct = "select distinct url from articles where Time_Publish > DATE_SUB(now(), INTERVAL 7 DAY) order by url ASC, Time_Check DESC\""
db_query_all = "select * from articles where Time_Publish > DATE_SUB(now(), INTERVAL 7 DAY) order by url ASC, Time_Check DESC\""
# get all distinct urls stored in the DB
db_connect = db_identify + db_authenticate + db_query_distinct + " > ./DB_urls.xml"
os.system(db_connect)
